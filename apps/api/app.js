...
app.post('/admin/series', async (req, res) => {
  const { name, description, price_jpy, total_slots, start_date, end_date, status, animation_video_url } = req.body;
  
  try {
    const result = await pool.query(
      `INSERT INTO series (name, description, price_jpy, total_slots, start_date, end_date, status, animation_video_url)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING *`,
      [name, description, price_jpy, total_slots, start_date, end_date, status || 'active', animation_video_url || null]
    );
...
, 
           description = // POST /admin/slots/:id/card, 
           price_jpy = $3, 
           total_slots = $4, 
           start_date = $5, 
           end_date = $6, 
           status = $7,
           animation_video_url = $8,
           updated_at = CURRENT_TIMESTAMP
       WHERE series_id = $9
       RETURNING *`,
      [name, description, price_jpy, total_slots, start_date, end_date, status, animation_video_url || null, id]
    );
...
app.delete('/admin/series/:id', async (req, res) => {
 ...
 RETURNING *',
    ...
    res.json({ message: 'Series deleted successfully', series: result.rows[0] });
  } catch (error) {
